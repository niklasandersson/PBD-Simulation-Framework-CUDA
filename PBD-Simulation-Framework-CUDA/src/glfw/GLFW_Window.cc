...
GLFW_Window::GLFW_Window(const unsigned int window_width, 
...
void GLFW_Window::render() {
  ...
  if( glfwGetKey(glfw_window_, GLFW_KEY_ESCAPE ) == GLFW_PRESS ||
      glfwWindowShouldClose(glfw_window_) == 1 ) {
    shouldClose_ = true;
  }
...
void GLFW_Window::toggleFreeze() {
  if( updateTime_ ) {
    freezeTime_ = current_time_;
  } else {
    glfwSetTime(freezeTime_);
  }
