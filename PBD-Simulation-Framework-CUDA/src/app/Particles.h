...
public:
  ...
  void render() override {
   ...
   //  }
    ...
    // L[3][3] = 0.0;
    // glm::mat4 rotation_matrix = glm::inverse(L);
    ...
    // std::cout << "*numberOfParticles_ = " << *numberOfParticles_ << std::endl;

    // glDrawArrays(GL_POINTS, 0, *numberOfParticles_);

    // glDrawElementsInstanced(GL_TRIANGLES, indices_.size(), GL_UNSIGNED_SHORT, nullptr, *numberOfParticles_);
    // glDrawElementsInstanced(GL_TRIANGLES, indices_.size(), GL_UNSIGNED_SHORT, nullptr, positons_.size());
